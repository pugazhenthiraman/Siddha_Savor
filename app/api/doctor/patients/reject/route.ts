...
export const POST = withErrorHandler(async (request: NextRequest) => {
  try {
    ...
    // Store email before updating
    const emailToNotify = patientEmail;

    // Mark patient as rejected (soft delete) instead of hard delete
    // This allows them to re-register and be moved back to pending
    // Store rejection info in formData and clear inviteToken to null (so they can re-register)
    ...
    // Keep a rejection marker token (unique) so they can be identified as rejected
    // When they re-register, this will be replaced with a new pending token
    const rejectionToken = `rejected_${patientId}_${Date.now()}`;
    
    await prisma.patient.update({
      where: { id: patientId },
      data: {
        inviteToken: rejectionToken, // Mark as rejected with special token
        formData: updatedFormData,
        updatedAt: new Date(),
      }
    });

    logger.info('Patient rejected', { patientId, patientEmail, doctorUID: patient.doctorUID, reason });

    // Send rejection email using admin SMTP configuration
    try {
            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <div style="background: #eff6ff; border-left: 4px solid #3b82f6; padding: 15px; margin: 20px 0;">
                ...
                <p style="margin: 15px 0 0 0; text-align: center;">
                  <a href="${baseUrl}/register?role=patient&email=${encodeURIComponent(emailToNotify)}" 
      ...
      if (!emailResponse.ok) {
        logger.warn('Failed to send rejection email', { patientEmail: emailToNotify, status: emailResponse.status });
      } else {
        logger.info('Rejection email sent successfully', { patientEmail: emailToNotify });
      }
    } catch (emailError) {
      // Log error but don't fail the rejection
      logger.error('Error sending rejection email', emailError, { patientEmail: emailToNotify });
    }
